...
export function RESTrequest(elements: THttpRequestParams): void {
	let editedUrl: string = (elements.url.indexOf("http") == 0 ? elements.url :
			"https://" + elements.url);
	if (elements.setDigest && elements.setDigest == true) {
 		let match: RegExpMatchArray = editedUrl.match(/(.*\/_api)/) as RegExpMatchArray;
 ...
 */
 export function findObjectPartByProperty(
	...
	if (Array.isArray(obj) == true)
		 result = searchArray(obj, property, allInstances);
	else
		 result = searchObject(obj, property, allInstances);
	return result;
}

function findObjectPropertyValue(obj: any, value: any): any {
...
function searchObject(
	...
	let result: any, collectedResults: any[] = [ ];
    ...
    for (let prop in obj) {
        ...
        if (result && (result.length && result.length >= 1 && result[0] != null) || result != null)
